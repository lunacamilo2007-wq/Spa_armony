                    ...
                    <div class="card p-6 animate-fade-in" id="cita-{{ $cita->id_cita }}">
                        <div class="flex flex-col lg:flex-row lg:items-center gap-4">
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        @if($cita->servicios->first())
                                            <span class="text-sm text-gray-500">· {{ $cita->servicios->first()->nombre_servicio }}</span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="text-xs text-gray-400 mt-1">
                                        Cédula: {{ $cita->cliente->cedula ?? '' }} · Tel: {{ $cita->cliente->telefono ?? 'N/A' }} · Email: {{ $cita->cliente->correo ?? 'N/A' }}
                                    </div>
                                    {{-- Service tags --}}
                            ...
                            <div class="flex items-center gap-4 lg:flex-col lg:items-end shrink-0">
                                ...
                                <span class="text-lg font-bold text-gray-900">${{ number_format($cita->total, 0, ',', '.') }}</span>

                                {{-- Actions Dropdown --}}
                                <div x-data="{ open: false }" class="relative">
                                    <button @click="open = !open" class="p-2 rounded-lg text-gray-400 hover:bg-gray-100 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"/></svg>
                                    </button>
                                    <div x-show="open" @click.outside="open = false" x-transition
                                         class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-10">
                                        @if($cita->estado === 'pendiente')
                                            <form method="POST" action="{{ route('citas.confirm', $cita->id_cita) }}">
                                                @csrf @method('PATCH')
                                                <button type="submit" class="w-full text-left px-4 py-2 text-sm text-primary-600 hover:bg-gray-50">✓ Confirmar</button>
                                            </form>
                                        @endif
                                        @if(in_array($cita->estado, ['pendiente', 'confirmada']))
                                            <form method="POST" action="{{ route('citas.cancel', $cita->id_cita) }}">
                                                @csrf @method('PATCH')
                                                <button type="submit" class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-gray-50">✕ Cancelar</button>
                                            </form>
                                            <form method="POST" action="{{ route('citas.finalize', $cita->id_cita) }}">
                                                @csrf @method('PATCH')
                                                <button type="submit" class="w-full text-left px-4 py-2 text-sm text-blue-600 hover:bg-gray-50">✓ Finalizar</button>
                                            </form>
                                        @endif
                                        <form method="POST" action="{{ route('citas.destroy', $cita->id_cita) }}" onsubmit="return confirm('¿Eliminar esta cita?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-gray-50">🗑 Eliminar</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
